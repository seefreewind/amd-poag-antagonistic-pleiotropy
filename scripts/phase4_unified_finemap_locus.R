...
locus <- if (length(args) >= 2) toupper(args[[2]]) else "L007"
if (!locus %in% c("L006", "L007")) stop("locus must be L006 or L007")

phase4 <- file.path(project, "results/phase4")
outdir <- file.path(phase4, paste0(locus, "_unified_susie"))
...
if (locus == "L006") {
  start <- 28100711L; end <- 30130300L
} else {
  start <- 32605227L; end <- 33611247L
}

ld <- fread(file.path(phase4, paste0(locus, "_QUANT_LD_MATRIX.tsv.gz")), showProgress = FALSE)
snps <- as.character(ld[[1]])
R <- as.matrix(ld[, -1, with = FALSE])
storage.mode(R) <- "double"
rownames(R) <- colnames(R) <- snps

h <- fread(file.path(phase4, paste0(locus, "_QUANT_LD_HARMONIZATION.tsv")), na.strings = c("", "NA"), showProgress = FALSE)
...
if (nrow(common) < 50) stop(locus, " common aligned SNP count is <50")
idx <- match(common$variant, snps)
Rk <- R[idx, idx, drop = FALSE]
min_eig <- min(eigen(Rk, symmetric = TRUE, only.values = TRUE)$values)
if (!is.finite(min_eig) || min_eig < -1e-6) stop(locus, " quantitative LD matrix failed PSD check: ", min_eig)
...
saveRDS(list(AMD = s1, POAG = s2, snp = common$variant, LD = Rk, dataset1 = d1, dataset2 = d2),
        file.path(outdir, paste0(locus, "_unified_susie_coloc_input.rds")))

fwrite(rbindlist(list(write_susie_summary("AMD", s1, common$variant), write_susie_summary("POAG", s2, common$variant))),
       file.path(phase4, paste0(locus, "_UNIFIED_SUSIE_SUMMARY.tsv")), sep = "\t", na = "NA")
...
if (status == "PASS") {
  ...
  saveRDS(cs, file.path(outdir, paste0(locus, "_unified_coloc_susie.rds")))
  pr <- posterior_row(cs)
  if (!is.null(pr)) {
  ...
  if (length(a) && length(b)) {
    ...
    if (length(signal_shared)) {
      score <- common$beta_ref_AMD[signal_shared] * common$beta_ref_POAG[signal_shared]
      top_i <- which.max(abs(score))
      top <- signal_shared[top_i]
      coloc_row$top_shared_variant <- common$variant[top]
      coloc_row$direction <- ifelse(score[top_i] < 0, "OPPOSITE", "CONCORDANT")
    }
  }
} else {
...
fwrite(coloc_row, file.path(phase4, paste0(locus, "_UNIFIED_COLOC.tsv")), sep = "\t", na = "NA")
write_json(list(
  ...
  locus = locus,
  note = paste0("Unified ", locus, " analyses use the quantitative EUR matrix after Phase 4 QC PASS; no Phase 0-3.5 discovery rerun was performed.")
), file.path(phase4, paste0(locus, "_UNIFIED_ANALYSIS_STATUS.json")), auto_unbox = TRUE, pretty = TRUE)
cat("phase4_unified_finemap_locus_complete", locus, "\n")
